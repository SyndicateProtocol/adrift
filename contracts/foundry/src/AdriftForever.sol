...
contract AdriftForever is AccessControl {
    ...
    function register(address player) public {
        require(nextCheckIn[player] == 0, "Player already registered");
        uint256 nextCheckInTime = block.timestamp + CHECKIN_INTERVAL;
        nextCheckIn[player] = nextCheckInTime;
        playerCount++;
        ...
        emit PlayerRegistered(player, block.timestamp, nextCheckInTime);
    }
